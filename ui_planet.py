...
from ui_common import _font, _fmt_time, Button
...
class PlanetUI:
    ...
    def _draw_ships_tab(self, surface, pr, y, p):
        ...
        for stype, defn in all_types:
            ...
            if unlocked:
                ...
                btn2 = Button((right - 76, ry + 42, 72, 18),
    ...
    def _draw_fleet(self, surface, pr, y, p, patrol_mode_ship=None):
        ...
        for ship in p.ships:
            ...
            # Line 1 – name + upgrade level + dock badge
            ...
            if here:
                ...
                if mission_type_now == "transport" and ship.state in ("travel", "returning"):
                    ...
                    badge_txt = "[A→B]" if leg == "out" else "[B→A]"
                    if ship.state == "travel" and tp:
                        st_txt = f"Transport → {tp.name} {badge_txt}"
                    else:
                        st_txt = f"Retour {badge_txt}"
                    st_col = CYAN
